...
def ensure_langchain_available():
    if LANGCHAIN_IMPORT_ERROR is not None:
        raise ModuleNotFoundError(
            "LangChain is required to run DIN-SQL generation. Install `langchain` and provider dependencies."
...
def build_prompt_bundle() -> Dict[str, ChatPromptTemplate]:
    ensure_langchain_available()
    ...
    medium_prompt = ChatPromptTemplate.from_messages([system_medium_prompt, human_medium_prompt])
    ...
    hard_prompt = ChatPromptTemplate.from_messages([system_hard_prompt, human_hard_prompt])
    ...
    return {
        "schema_linking": schema_linking_prompt,
...
def run_din_sql_case(
    ...
    prompt_bundle: Optional[Dict[str, ChatPromptTemplate]] = None,
) -> Dict[str, Optional[str]]:
    """Run DIN-SQL for one input and return all intermediate outputs + final query."""
    ensure_langchain_available()
    prompts = prompt_bundle or build_prompt_bundle()

    schema_linking = LLMChain(llm=chat_model, prompt=prompts["schema_linking"]).run(
        question=question,
        schema=schema,
        hint=hint,
    ...
    classification = LLMChain(llm=chat_model, prompt=prompts["classification"]).run(
        question=question,
        schema=schema,
        hint=hint,
        columns_descriptions=columns_descriptions,
    ...
    if "EASY" in label:
        sql_generation = LLMChain(llm=chat_model, prompt=prompts["easy"]).run(
            question=question,
            schema=schema,
            hint=hint,
            columns_descriptions=columns_descriptions,
        ...
        sql_generation = LLMChain(llm=chat_model, prompt=prompts["medium"]).run(
            question=question,
            schema=schema,
            hint=hint,
            columns_descriptions=columns_descriptions,
        ...
        sql_generation = LLMChain(llm=chat_model, prompt=prompts["hard"]).run(
            question=question,
            schema=schema,
            hint=hint,
            columns_descriptions=columns_descriptions,
    ...
    sql_query = extract_sql_query(sql_generation)

    correction = LLMChain(llm=chat_model, prompt=prompts["correction"]).run(
        question=question,
        schema=schema,
        columns_descriptions=columns_descriptions,
        hint=hint,
    ...
    revised_sql = extract_revised_sql_query(correction)
    if revised_sql is not None:
        final_query = revised_sql.replace('\n', '').replace('\r', '')
    elif sql_query is not None:
        final_query = sql_query.replace('\n', '').replace('\r', '')
    else:
...
def generate_sql_query(
    ...
    """Convenience API for other modules: pass schema + prompt context, get final SQL."""
    result = run_din_sql_case(
...
def run_din_sql_dataframe(
    ...
    """Original DIN-SQL loop, moved into a reusable function."""
    ...
    for index, row in dev_df.iterrows():
        ...
        db_descriptions = dev_db_path + "/" + row["db_id"] + "/" + "database_description"
        ...
        update_json_file(prediction_json_path, index, result["final_query"], row["db_id"])

        print("final sql query: ", result["final_query"])
        print("Gold sql query: ", row.get("SQL", ""))
        print("--------------------------------------------------")
...
def main():
    ...
    run_din_sql_dataframe(
        ...
        prediction_json_path="predict_dev.json",
    )
