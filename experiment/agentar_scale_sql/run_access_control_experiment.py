...
4) view_filtered_access_control
5) prompt_filtered_access_control
"""
...
def aggregate_mode_scores(contrib_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if base_group is not None and not base_group.empty:
        ...
        # intent-preservation match (score == 1.0).
        epsilon = 1e-9
...
def build_question_type_breakdown(eval_df: pd.DataFrame) -> pd.DataFrame:
    ...
    working_df["execution_scored_num"] = working_df.apply(
        lambda row: _safe_binary(row.get("execution_scored", 1)),
        axis=1,
    )
    working_df["syntax_scored_num"] = working_df.apply(
        lambda row: _safe_binary(row.get("syntax_scored", 1)),
        axis=1,
    )
    working_df["execution_success_num"] = working_df.apply(
        lambda row: (
            _safe_binary(row.get("execution_success", 0))
            if _safe_binary(row.get("execution_scored", 1)) == 1
            else 0
        ),
        axis=1,
    )
    working_df["syntactic_correct_num"] = working_df.apply(
        lambda row: (
            _safe_binary(row.get("syntactic_correct", 0))
            if _safe_binary(row.get("syntax_scored", 1)) == 1
            else 0
        ),
        axis=1,
    )
    working_df["policy_violation_binary_num"] = working_df.apply(
        lambda row: _safe_binary(
            row.get(
                "policy_violation_binary",
                1 if _safe_int(row.get("policy_violation_count", 0)) > 0 else 0,
            )
        ),
        axis=1,
    )
    working_df["prompt_char_size_num"] = working_df.apply(
        lambda row: (
            _safe_int(row.get("prompt_char_size", 0))
            if _safe_binary(row.get("prompt_char_size_scored", 0)) == 1
            else 0
        ),
        axis=1,
    )
    working_df["prompt_char_size_scored_num"] = working_df.apply(
        lambda row: _safe_binary(row.get("prompt_char_size_scored", 0)),
        axis=1,
    )

    grouped = (
        working_df.groupby(["access_control_mode", "question_type"], dropna=False)
        .agg(
            sample_count=("question_id", "count"),
            mean_policy_violation_rate=("policy_violation_rate_row", "mean"),
            mean_policy_violation_binary=("policy_violation_binary_num", "mean"),
            mean_direct_disclosure_rate=("direct_disclosure_rate_row", "mean"),
            execution_success_sum=("execution_success_num", "sum"),
            execution_scored_count=("execution_scored_num", "sum"),
            syntactic_correct_sum=("syntactic_correct_num", "sum"),
            syntax_scored_count=("syntax_scored_num", "sum"),
            prompt_char_size_sum=("prompt_char_size_num", "sum"),
            prompt_char_size_scored_count=("prompt_char_size_scored_num", "sum"),
        )
        .reset_index()
    )
    grouped["mean_execution_success"] = grouped.apply(
        lambda row: (
            float(row["execution_success_sum"]) / float(row["execution_scored_count"])
            if float(row["execution_scored_count"]) > 0.0
            else 0.0
        ),
        axis=1,
    )
    grouped["mean_syntactic_correctness"] = grouped.apply(
        lambda row: (
            float(row["syntactic_correct_sum"]) / float(row["syntax_scored_count"])
            if float(row["syntax_scored_count"]) > 0.0
            else 0.0
        ),
        axis=1,
    )
    grouped["mean_prompt_char_size"] = grouped.apply(
        lambda row: (
            float(row["prompt_char_size_sum"]) / float(row["prompt_char_size_scored_count"])
            if float(row["prompt_char_size_scored_count"]) > 0.0
            else 0.0
        ),
        axis=1,
    )
    return grouped[
        [
            ...
            "question_type",
            "sample_count",
            "mean_policy_violation_rate",
            "mean_policy_violation_binary",
            "mean_direct_disclosure_rate",
            "mean_execution_success",
            "mean_syntactic_correctness",
            "mean_prompt_char_size",
            "execution_scored_count",
            "syntax_scored_count",
            "prompt_char_size_scored_count",
        ]
    ].sort_values(["access_control_mode", "question_type"])
...
def _argos_failure_reasons(eval_row: Dict[str, Any]) -> List[str]:
    if str(eval_row.get("access_control_mode", "")).strip() != "argos_access_control":
        return []
    ...
    try:
        for sample_position in range(processed_samples, len(exp_df)):
            ...
            for mode_output in mode_outputs:
                ...
                if mode_name == "argos_access_control":
                    ...
                    if isinstance(argos_meta, dict):
                        ...
                        if argos_error:
                            print(
                                f"[warn] question_id={sample.get('question_id')} "
                                f"mode=argos_access_control argos_error={argos_error}"
                            )
