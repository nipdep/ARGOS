#!/usr/bin/env python3
"""
Build deterministic question/query pairs for the true filter-only track.

Input (per DB):
- qa_config_filter_only.json (templates + role_copies)
- schema.json (foreign keys for join construction)

Output (per DB):
- pq_pairs_filter_only.json
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build deterministic filter-only QA pairs from filter-only config resources."
    )
    parser.add_argument(
        "--base-dir",
    ...
    parser.add_argument(
        "--output-name",
        default="pq_pairs_filter_only.json",
        help="Per-DB output file name.",
    )
    parser.add_argument(
        "--combined-output",
        default="pq_pairs_filter_only_all.json",
...
def build_query_for_mode(
    *,
    ...
    if mode == "group_by":
        ...
        group_exprs = [col_sql(c, aliases) for c in group_cols]
        ...
        # If overlap is unavoidable, we still keep only one version in SELECT
        # (raw projection OR aggregated expression), never both.
        non_overlap_not_group = [
            c for c in kept_process if c not in projection_set and c not in group_set
        ]
        non_overlap_group = [
            c for c in kept_process if c not in projection_set and c in group_set
        ]
        overlap_not_group = [
            c for c in kept_process if c in projection_set and c not in group_set
        ]
        overlap_group = [
            c for c in kept_process if c in projection_set and c in group_set
        ]

        agg_cols: list[str] = []
        if non_overlap_not_group or non_overlap_group:
            agg_buckets = (non_overlap_not_group, non_overlap_group)
        else:
            agg_buckets = (overlap_not_group, overlap_group)
...
def build_question_text(
    ...
    if mode == "aggregate":
        if not used_projection_columns:
            return (
                f"Across all rows in {table_part}, compute aggregate metrics for "
                f"{proc_part} without WHERE or GROUP BY."
            )
        return (
            f"Across all rows in {table_part}, keep {proj_part} and compute aggregate "
            f"metrics for {proc_part} without WHERE or GROUP BY."
        )
...
def summarize_projection_restriction_drops(records: list[dict[str, Any]]) -> int:
    dropped = 0
    for record in records:
        for role_data in record.get("role_expected", {}).values():
            if role_data.get("dropped_restricted_projection"):
                dropped += 1
    return dropped
...
def main() -> None:
    ...
    for db_dir in db_dirs:
        ...
        projection_drops = summarize_projection_restriction_drops(result["records"])
        print(
            f"[{result['db_id']}] cases={total} base_usable={usable} "
            f"projection_restricted_drops={projection_drops}"
        )
